...
The two bare engine includes `rg.nvgt` relies on — `bgt_compat.nvgt` and `sound_pool.nvgt` — resolve from the new engine's include folder: `nvgt_0.90.0_dev\release\include\`. Run a script with `nvgt rg.nvgt`; debug with `nvgt -d rg.nvgt`. See [[rhythm-rage-bgt-to-nvgt-port]].
