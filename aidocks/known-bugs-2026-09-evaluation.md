...
3. **NOT A BUG, intentional (dev confirmed 2026-09-22):** the original author, Oriol Gomez, made the tutorial start sound replay at the end on purpose. Leave the pass-2 `tutes.play_wait()` alone and don't re-report it. Original report: **Tutorial start sound likely replays at tutorial end.** The parser's two-pass loop (`for (uint i=1;i<3;i++)`, ~963): on pass 2 `tutorial` is still true, so `tutes.play_wait()` (~966) runs again before the empty-string `break`, then `loadlev` plays `tut_end`. Possibly BGT-faithful — confirm with the dev before "fixing".
4. **FIXED (2026-09-22)**: the loop now preloads every file, skipping already-loaded ones with `continue` instead of returning, and returns false if any failed. In creator mode a missing sound anywhere in the list now alerts at parse time. Logged in 1.4. Original report: **`preload_add` only preloads the first of a comma list.** `includes/classes.nvgt` ~30: `return true;` sits inside the per-file loop, so `name=a,b` never preloads `b` (loaded on first play → possible mid-level hitch). The duplicate-check `return false` also aborts the rest of the list.
5. **FIXED (2026-09-22), logged in 1.5**: time alerts print `lines[curline-1]` (the play-branch one now also `exit()`s instead of alerting twice), macro element counts are checked before `words[1]`/`words[2]` are read, `seek` reads `words[1]`, and the macro test is `(… or …) && !tutorial` (no shipped `.tut` uses macros). Follow-up, also FIXED 2026-09-22 (same 1.5 entry): `check_value()` alerts when one of the 23 value-reading setting names (`valued_settings`) has no `=value`. Unknown bare words are still ignored, so no existing pack can newly fail. `check_parts()` guards `interactive` setup (6 parts), `intersound` (5+, plus a bare `key` token), and tutorial `wait`/`playwait`/`music` (2+). If a new name=value setting is added to the parser, add it to `valued_settings` too. Original report: **Level-script typos crash (index out of range) instead of showing the friendly alert**, author-facing only:
