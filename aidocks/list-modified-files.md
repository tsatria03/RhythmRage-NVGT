...
**How to apply:** End each response that performed edits with a short "Files changed:" list covering every file written, edited, or deleted that turn — including doc and config files, not just code. One line per file, bare filenames only (e.g. `rg.nvgt`, `utils.nvgt`, `changelog.txt`) — no directory paths. RhythmRage is a single game (no client/server split), so no side tags are needed; if two touched files share a name across folders, add just enough to disambiguate. Related: [[confirm-before-implementing]], [[no-crlf-normalization]].
