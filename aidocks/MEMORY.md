...
- [Changelog rules](changelog-rules.md) — docks/english/changelog.txt: player-facing prose, 1-3 sentence entries, per-version caps (20/.0, 10/minor), reverse-chronological; released blocks (1.0–1.4) are frozen; only unreleased 1.5 is editable
...
- [Code structure plan](code-structure-plan.md) — 5 ranked refactors (key lookup, split startlev/loadlev, say2 helpers, shared list browser, screen loop) with fixes, risks, tests and status; nothing implemented yet
- [Known bugs (2026-09 evaluation)](known-bugs-2026-09-evaluation.md) — bug list with status: save prefix-match mixup (01/01a, FIXED), tutorial busy-wait (FIXED), lt tap-tempo hang (FIXED), preload early return (FIXED), tutorial start sound at end (intentional, not a bug); parser typo crashes (#5, FIXED incl. missing =value and short command lines; new settings must join valued_settings), non-atomic save (FIXED: .tmp/.bak rotation + fallback load); timing proposal A (keycal shifts) and B (nearest-beep median calibration) DONE, C (nearest-note judging) open, preload early return, parser crash paths, timing/save risks
