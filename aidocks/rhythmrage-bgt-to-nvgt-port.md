...
- The `mypacks/` folder is NOT read by the game (BGT or NVGT) — it's a holding area for editable source folders; you must copy a folder into `packs/` to build/test it in the game. The game allows only ONE loose folder in `packs/` at a time (generate_packs alerts "Multiple packs" otherwise). NOTE: the separate `leveltool.nvgt` authoring tool DOES scan `mypacks/` for its pack picker — you find timings there, then copy that one pack into `packs/` to test-run a level via the tool's `r` key.

**Layout (now a git repo, remote https://github.com/tsatria03/RhythmRage-NVGT.git):** local includes live in `includes/` (renamed from `include/`); `game.nvgt`'s six `#include "includes/..."` lines match. `bgt_compat.nvgt` and `sound_pool.nvgt` stay bare (resolved from the engine include path). Changelog lives at `docks/changelog.txt` (terse one-liners under "New in 1.0.").

**Sound packs relocated to `data/assets/`:** `sounds1.pack`/`sounds2.pack` (the built voice/UI sound packs) now live in `data/assets/`, not the root. Code references were updated accordingly — `set_sound_storage`/`package.create` use `path+"data/assets/sounds"+lang+".pack"` (game.nvgt `main()` + `generate_soundpack()`, and `nopack()` in `includes/utils.nvgt`). `main()` also `directory_create(path+"data/assets")`. The raw `sounds/` source folder (packed by `generate_soundpack()` via `find_files("sounds/*")`), `packs/`, and `save/` all remain in the ROOT — do not prefix those with data/assets. `.gitignore` excludes all binary media (`*.pack`, `sounds/`, `mypacks/`, `packs/`) so a fresh clone is code-only; `generate_soundpack()` then finds an empty `sounds/` and returns without building. Note: `.gitignore` lists `lib/` and `release/` but the actual folders are `libs/` and `releases/` (mismatch, currently empty so harmless).
