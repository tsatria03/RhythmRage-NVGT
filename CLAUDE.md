...
- **`rg/`** — the game's data/run folder (ships alongside the compiled exe). Its **`data/`** holds only `data/assets/` (`sounds1.pack`/`sounds2.pack`, the built voice/UI sound packs, one per language) and `data/saves/` (`rg.dat`, the portable profile, plus `rg.dat.tmp`/`rg.dat.bak` from the safe-save swap). **Packs are not in `rg/`**; they live in app data (see "Pack system" below). Also `mypacks/` (loose author pack sources, read by the level tool **and** by the game's test/compile/decompile-a-pack menu items and its first-run default-pack build), `sounds/` (raw source for building the sound packs into `data/assets/`), `docks/` (player-facing docs, split into `english/` and `spanish/` subfolders — includes the level-script authoring reference `docks/english/parser.md`), and the Python launchers `rg.py`/`lt.py`. No `lib/` — the compiler supplies the platform libraries (**[[nvgt-090-miniaudio-not-bass]]**).
- **`build/`** — `tools.py`/`tools.bat`/`tools.ini`: a menu with git helpers plus **Compile** and **Package** for Windows + Mac.
- **`releases/`** — gitignored build output: `windows/RhythmRage_windows/rg/` (game + `lt.exe` + a `mypacks/` holding only the `default`/`default_espanol` sources, for the offline default-pack build), `mac/RhythmRage_mac/rg.app`, and `archives/*.zip`.
...
- **Pack system.** All playable content lives in encrypted `.pack` files (levels `.lvl`, tutorials `.tut`, `.ogg`; decryption key `<packname>guillemandoriolftw`) in a **fixed app-data folder**: `packdir` = `%APPDATA%\Oriol Gomez\RhythmRage\packs\`, set in `main()`. It's the same folder whether the game runs from source, portable, or installed, so packs don't travel with the game folder. `main()` opens the current `.pack` (default `default`). On a first run with no packs at all, `ensure_default_packs()` builds `default`/`default_espanol` from `mypacks/`, or downloads them if the sources are missing. Authors work from `mypacks/` through the main menu's **test / compile / decompile a pack** items. Testing builds a throwaway copy in the temp folder and runs it in creator mode (all levels unlocked, nothing saved). The older BGT-style flow also still works: a **loose folder** dropped into `packdir` is built by `generate_packs()` at launch and opened in creator mode. See **[[rhythm-rage-bgt-to-nvgt-port]]**.
